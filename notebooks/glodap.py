...
def _gen_v1_dataset(clobber=False):
    ...
    if os.path.exists(netcdf_file):
        ...
        dy = 1
        lon0 = -180.0
        lat = xr.DataArray(np.arange(-90.0 + dy / 2.0, 90.0, dy), dims=("lat"))
        lon = xr.DataArray(np.arange(lon0 + dx / 2.0, lon0 + 360.0, dx), dims=("lon"))
        ds = xr.Dataset(
            dict(
                lon=lon,
                lat=lat,
            ...
            with open(file_in, "r") as fid:
                ...
                for k in range(nk):
                    for i in range(ni):
                        line = fid.readline()
                        data[k, :, i] = [
                            convert_str(v.strip()) for v in line.split(",")
                        ]
                        n += 1
...
def _gen_Gruber2019_dataset():
    ...
    ndx0 = np.where(ds.lon > 180.0)[0]
    ndx1 = np.where(ds.lon < 180.0)[0]
...
def open_glodap_pop_grid(product="GLODAPv1", model_grid="POP_gx1v7", method="bilinear"):
    ...
    if "GLODAPv1" in product:
        src_grid = regrid_tools.grid(
            "latlon_glodapv1", nx=360, ny=180, lon0=-180.0
        )  # , grid_imask=grid_imask)
    else:
        src_grid = regrid_tools.grid(
            "latlon_glodapv2", nx=360, ny=180, lon0=20.5
        )  # , grid_imask=grid_imask)

    regridder = regrid_tools.regridder(src_grid, dst_grid, method, clobber=False)

    spatial_vars = [
        v for v in ds_src.data_vars if ("lat", "lon") == ds_src[v].dims[-2:]
    ]
    ...
    for v in ds_dst_xy.data_vars:
        ...
        if zdim not in da.dims:
            ...
            with xr.set_options(keep_attrs=True):
                ...
                da_out = xr.where(np.isclose(da_out, 0.0, atol=1e-10), 0.0, da_out)
