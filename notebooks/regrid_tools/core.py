...
class grid(object):
    ...
    def __init__(self, grid_name, clobber=False, grid_file=None, **kwargs):
        ...
        if grid_file is None:
            if 'latlon' in grid_name:
                assert all(k in kwargs for k in ['nx', 'ny', 'lon0'])
                dx = 360.0 / kwargs['nx']
                dy = 180.0 / kwargs['ny']
                lon0 = float(kwargs['lon0'])
                self.grid_name = f'{grid_name}_{grid_name}_{dx}x{dy}_lon0={lon0}'
                self.grid_file = f'{regrid_dir}/{grid_name}_{dx}x{dy}_lon0={lon0}.nc'
            else:
    ...
    def __repr__(self):
        return f'grid: {self.grid_name}\ndims: {self.dims}\nfile: {self.grid_file}'


def generate_latlon_grid(nx, ny, lon0=0.0):
    """
    ...
    lon = np.arange(lon0 + dx / 2.0, lon0 + 360.0, dx)

    yc = np.broadcast_to(lat[:, None], (ny, nx))
    xc = np.broadcast_to(lon[None, :], (ny, nx))

    yv = np.stack((yc - dy / 2.0, yc - dy / 2.0, yc + dy / 2.0, yc + dy / 2.0), axis=2)
    xv = np.stack((xc - dx / 2.0, xc + dx / 2.0, xc + dx / 2.0, xc - dx / 2.0), axis=2)

    y0 = np.sin(yv[:, :, 0] * deg2rad)  # south
    y1 = np.sin(yv[:, :, 3] * deg2rad)  # north
    x0 = xv[:, :, 0] * deg2rad  # west
    x1 = xv[:, :, 1] * deg2rad  # east
    area = (y1 - y0) * (x1 - x0) * Re ** 2.0
    ...
    ds['xc'] = xr.DataArray(
        xc,
        dims=('lat', 'lon'),
        attrs={'units': 'degrees_east', 'long_name': 'longitude of cell centers'},
    )

    ds['yc'] = xr.DataArray(
        yc,
        dims=('lat', 'lon'),
        attrs={'units': 'degrees_north', 'long_name': 'latitude of cell centers'},
    )

    ds['xv'] = xr.DataArray(
        xv,
        dims=('lat', 'lon', 'nv'),
        attrs={'units': 'degrees_east', 'long_name': 'longitude of cell corners'},
    )

    ds['yv'] = xr.DataArray(
        yv,
        dims=('lat', 'lon', 'nv'),
        attrs={'units': 'degrees_north', 'long_name': 'latitude of cell corners'},
    )

    ds['area'] = xr.DataArray(
        area, dims=('lat', 'lon'), attrs={'units': 'm^2', 'long_name': 'area'}
    )
...
def latlon_to_scrip(
    nx=None, ny=None, lon=None, lat=None, lon0=-180.0, grid_imask=None, file_out=None
):
    ...
    # compute area
    y0 = np.sin(y_corner[:, :, 0] * np.pi / 180.0)  # south
    y1 = np.sin(y_corner[:, :, 3] * np.pi / 180.0)  # north
    x0 = x_corner[:, :, 0] * np.pi / 180.0  # west
    x1 = x_corner[:, :, 1] * np.pi / 180.0  # east
    grid_area = (y1 - y0) * (x1 - x0)

    # sum of area should be equal to area of sphere
    np.testing.assert_allclose(grid_area.sum(), 4.0 * np.pi)
    ...
    dso = xr.Dataset()
    dso['grid_dims'] = xr.DataArray(np.array([nx, ny], dtype=np.int32), dims=('grid_rank',))
    ...
    dso['grid_corner_lat'] = xr.DataArray(
        y_corner.reshape((-1, 4)), dims=('grid_size', 'grid_corners'), attrs={'units': 'degrees'}
    )
    dso['grid_corner_lon'] = xr.DataArray(
        x_corner.reshape((-1, 4)), dims=('grid_size', 'grid_corners'), attrs={'units': 'degrees'}
    )
    ...
    dso.attrs = {
        'title': f'{dy} x {dx} (lat x lon) grid',
...
def esmf_regrid_weight_gen(
    src_grid_file, dst_grid_file, weight_file, method, extrap_method='none', clobber=False
):
...
class regridder(object):
    ...
    def __repr__(self):
        return f'regridder {os.path.basename(self.src_grid_file)} --> {os.path.basename(self.dst_grid_file)}'
    ...
    def _regrid_dataarray(self, da_in, renormalize=True, apply_mask=True):
        ...
        # remap the field
        data_dst = esmf_apply_weights(self.weights, data_src, self.dims_src, self.dims_dst)
        ...
        if renormalize:
            ...
            ones_dst = esmf_apply_weights(self.weights, ones_src, self.dims_src, self.dims_dst)
